...
import { trackWhatsAppClick, trackBookDemo, trackFormSubmit, trackPhoneClick } from "@/lib/traking";
...
📚 Course: ${form.course}
    ...
    trackBookDemo(`Course: ${form.course}`);
    trackWhatsAppClick("book-demo-form");
    trackFormSubmit("Free Demo Registration", "success");
  ...
  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-lg sm:p-8">
      ...
      <form onSubmit={handleSubmit} className="mt-6 space-y-4 sm:mt-8 sm:space-y-5">
        ...
        <button
          type="submit"
          className="w-full rounded-xl bg-yellow-400 px-6 py-3.5 font-semibold text-slate-900 transition-colors hover:bg-yellow-500 min-h-[44px]"
        >
        ...
        <button
          ...
          className="flex flex-wrap items-center justify-center gap-2 pt-2 text-center text-sm text-slate-500 w-full hover:text-slate-900 min-h-[44px]"
        >
          <FaPhoneAlt className="shrink-0 text-yellow-500" />
